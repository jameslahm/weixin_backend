...
public class Dispatcher {
    ...
    public String dispatch(CommonInParams params) {
        ...
        try {
            ...
            if (id != null && bitType != BizTypeEnum.USER_LOGIN) {
                User user = userProcessor.getUserById(id);
            ...
            if (isWarning) {
                ...
                /** 记录警告日志，并返回警告的参数 */
                LogUtil.WARN(id, bitType, params, courseWarn);
                return new SysWarnOutParams(courseWarn).toString();
            } else {
                /** 记录错误日志并返回服务器内部错误 */
                LogUtil.ERROR(id, bitType, params, realError);
